...
TO RUN:
    ...
    make sure you have the pl_examples folder in the same dir as this script
    
    python DL_benchmark.py --trainer.max_epochs=10 --trainer.gpus=0 << for the CPU test
    python DL_benchmark.py --trainer.max_epochs=10 --trainer.gpus=1 << for the GPU test
...
class ImageClassifier(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_acc = self.test_acc(logits,y)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val_acc", self.test_acc)
...
class MNISTDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        return torch.utils.data.DataLoader(train_dataset, batch_size=self.hparams.batch_size,num_workers=14)

    def val_dataloader(self):
        test_dataset = MNIST("./data", train=False, download=False, transform=self.transform)
        return torch.utils.data.DataLoader(test_dataset, batch_size=self.hparams.batch_size,num_workers=14)
